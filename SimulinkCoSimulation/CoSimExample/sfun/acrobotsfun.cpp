...
#define S_FUNCTION_NAME  acrobotsfun
...
static void mdlCheckParameters(SimStruct *S)
{
    ...
    isValid = mxIsDouble(INPUT_PARAM(S)) &&
        mxGetNumberOfElements(INPUT_PARAM(S)) == 1 &&
        !mxIsComplex(INPUT_PARAM(S));

    if (isValid){
        double *w = reinterpret_cast<double *>(mxGetData(INPUT_PARAM(S)));
        if (*w < 0) isValid = false;
        else{
             inputNum = static_cast<unsigned int>(*w);
        }
    }
    if (!isValid) {
    ...
    isValid = mxIsDouble(OUTPUT_PARAM(S)) && 
        mxGetNumberOfElements(OUTPUT_PARAM(S)) == 1 &&
        !mxIsComplex(OUTPUT_PARAM(S));

    if (isValid){
        double *w = reinterpret_cast<double *>(mxGetData(OUTPUT_PARAM(S)));
        if (*w < 0) isValid = false;
        else{
                outputNum = static_cast<unsigned int>(*w);
        }
    }
    if (!isValid) {
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetSFcnParamTunable(S, STEP_SIZE_P, true);
    ssSetSFcnParamTunable(S, INPUT_IDX, true);
    ssSetSFcnParamTunable(S, OUTPUT_IDX, true);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    try {
        outputs_wrapper(GET_ZM_PTR(S), iter_ptr, y1_ptr, y2_ptr);
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
    ...
    int_T port;
    double Input_data[inputNum] = {0} ; 

    for(port = 0; port < inputNum; port++)
    {
        Input_data[port] = (*(reinterpret_cast<const double *>(ssGetInputPortSignal(S,port))));
    }

    update_wrapper(GET_ZM_PTR(S), iter_ptr, Input_data, inputNum);      
}
